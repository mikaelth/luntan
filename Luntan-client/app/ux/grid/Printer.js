                ...
                '</html>',
                {
                    ...
                    renderColumn: function(column, value, rcd, col)
                    {
                    	...
                    	if (column.xtype == 'templatecolumn')
                    	{
                            ...
                            } else if (column instanceof Ext.grid.column.Check) {
                                value = column.renderer.call(column, value, meta, rcd, -1, col - 1, this.grid.store, this.grid.view);    // Mikaels hack                        
                            } else {
...
//            var fields   = grid.store.getProxy().getModel().getFields();
            var fields   =  grid.store.hasOwnProperty('source') ? grid.store.source.getProxy().getModel().getFields() : grid.store.getProxy().getModel().getFields();
